...
- 任务新增、编辑、完成、删除、收藏、标记置顶、归档、清理已完成
- 任务分组、重命名、删除，删除分组时任务自动回到收集箱
- 支持拖动任务到分组；侧边栏可筛选待办、逾期、等反馈、风险、收藏、标记、全部、已完成和归档
- PM 状态标记：待处理、等反馈、风险
- 顶部任务搜索，支持在当前视图内即时检索
- 提醒时间编辑、清除、逾期筛选和逾期高亮
...
- 给已有任务追加图片附件
- 紧凑任务卡：底部横向操作栏，任务有图片时显示一张缩略图
- 数据管理：任务、分组、设置与图片附件可备份和恢复；任务可导出为 CSV 或 Markdown
- 托盘菜单、全局快捷键、开机启动设置
...
通过设置页的“数据管理”可将以上数据和图片附件备份到任意文件夹；恢复前应用会自动保留一份恢复前备份。CSV / Markdown 导出会包含已归档任务。
